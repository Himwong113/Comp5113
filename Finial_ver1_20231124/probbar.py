...
def detect(im, model, transforms):
    ...
    #print(probas.shape)
    keep = probas.max(-1).values>0   #keep class >0.85 only
    #print(keep.shape)#
...
tenarr = plot_results(im,score,boxes)
tenarr_array = np.array(tenarr)



# Print the shape of the transposed array
print(tenarr_array.shape)

# Calculate the sum of each row (axis 1) in the transposed array
for tenarr_array_index in tenarr_array :

    cocoset= coco()

    index = [cocoset[f'{i}'] for i in range(1,92)]

    probinfo=tenarr_array_index.numpy()

    plt.bar(index,probinfo)
    plt.show()
